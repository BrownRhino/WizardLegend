var GameplayState =
{
	timer: null,//timer to test for difficulty
	timerText: null,
	elapsedTime: null,	
	...
	{
		this.timer++;
		this.timerText.text = 'Timer: ' + this.timer;
	},
...
//		game.state.start('LevelComplete');
				...
				else
				{
					...
					if (self.shootWaitTime > 1.25)
					{
						...
						if (newBullet != null)
						{
							...
							newBullet.lifespan = 2000; // the bullet will live for a number of milliseconds
						}
					}
				}
			});
	...
	create: function()
	{
		...
		//..........................................................Timer for game
		this.timer = 0;
		this.timerText = game.add.text(175, 8, 'timer: ' + this.timer, { font: '8px Conv_Gamegirl', fill: 'white' });
		this.timerText.smoothed = false;
		this.timerText.fixedToCamera = true;
		
		//set up timer
		this.elapsedTime = game.time.create(false);
		this.elapsedTime.loop(Phaser.Timer.SECOND, this.updateTimer, this);
		
		// spawn enemies
		
		this.numofshooters=Difficulty;
		this.numofjumpers=NumberOfEnemies-Difficulty-1;	
		
		for (var i = 1; i < NumberOfEnemies; i++)
		{
			var roll = Math.random() * 10;
						
			if ( (this.numofshooters == 0) || ((roll < 5) && (this.numofjumpers != 0)) )
			{
...
//		document.write('numofshooters is ' + this.numofshooters + ".  numofjumpers is " + this.numofjumpers+".    ");

/*		
		for (var i = 1; i < NumberOfEnemies - Difficulty; i++)
		{
			this.spawnJumper(i * 6, 2);
		}
			
		for (var i = 1; i < 1 + Difficulty; i++)
		{
			this.spawnShooter(i * 7, 2);
		}
*/
...
//		this.elapsedTime.loop(Phaser.Timer.SECOND, this.updateTimer, this);
		...
		if ((BButtonDown || game.input.keyboard.isDown(Phaser.Keyboard.X)) && this.player.isShootButtonDown == false && this.player.canShoot == true && this.player.knockedBack == false && PlayerHealth > 0)
		{
			...
			this.game.time.events.add(1000, function() { this.player.canShoot = true; }, this);
